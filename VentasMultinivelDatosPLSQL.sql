...
after insert or delete on PRODUCTO_VENTA
    FOR EACH ROW
DECLARE
    V_TOTAL NUMBER(15,2);
...
BEGIN
	IF INSERTING THEN
		...
		DBMS_OUTPUT.put_line (V_ANTERIOR);
		...
		select I.V_IVA INTO V_IVA from IVA I,VENTA V where V.K_IVA =I.K_IMPUESTO AND :new.K_VENTA=V.K_CODVENTA;
		DBMS_OUTPUT.put_line (V_IVA);
	...
	ELSE 
		SELECT V.V_PRECIOFINAL INTO V_ANTERIOR FROM VENTA V WHERE :OLD.K_VENTA=V.K_CODVENTA;
		DBMS_OUTPUT.put_line (V_ANTERIOR);
		SELECT (P.V_PRECIO * :OLD.Q_CANTIDAD) INTO V_TOTAL FROM PRECIO P,VENTA V WHERE P.K_PRODUCTO=:OLD.K_PRODUCTO AND :OLD.K_VENTA=V.K_CODVENTA;
		DBMS_OUTPUT.put_line (V_TOTAL);
		select I.V_IVA INTO V_IVA from IVA I,VENTA V where V.K_IVA =I.K_IMPUESTO AND :OLD.K_VENTA=V.K_CODVENTA;
		DBMS_OUTPUT.put_line (V_IVA);
		select RP.K_CODREGION INTO K_REGION from REPRESENTANTE_CLIENTE RC, VENTA V, REGION_REPRESENTANTE RP where V.K_ID_REP_CLIENTE=RC.K_ID_REP_CLIENTE AND V.K_CODVENTA = :OLD.K_VENTA AND RP.K_REPVENTA=RC.K_CODREPVENTAS;
		DBMS_OUTPUT.put_line (K_REGION);
		V_TOTAL := V_ANTERIOR - (V_TOTAL * V_IVA);
		UPDATE VENTA SET V_PRECIOFINAL = V_TOTAL WHERE VENTA.K_CODVENTA = :OLD.K_VENTA;
		UPDATE PRODUCTO_REGION PR SET Q_STOCK = Q_STOCK - :OLD.Q_CANTIDAD WHERE PR.K_PRODUCTO = :OLD.K_PRODUCTO AND PR.K_REGION = K_REGION;
	END IF;
END;
/
